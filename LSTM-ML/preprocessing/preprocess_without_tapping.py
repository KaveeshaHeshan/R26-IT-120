print("Preprocessing without tapping dataset started...")
...
for df in [latex_data, farmer_data, weather_data]:
...
if 'storage_duration_hours' in merged_data.columns:
    merged_data['storage_risk'] = (
        merged_data['storage_duration_hours'] > 6
    ).astype(int)
else:
    merged_data['storage_risk'] = 0

if 'tapping_hour' in merged_data.columns and 'tapping_minute' in merged_data.columns:
...
temp_col = None
for col in ['temperature_c_x', 'temperature_c', 'temp_mean']:
    if col in merged_data.columns:
        temp_col = col
        break

humidity_col = None
for col in ['humidity_percent', 'humidity_max']:
    if col in merged_data.columns:
        humidity_col = col
        break

if temp_col and humidity_col:
    merged_data['weather_stress'] = (
...
final_data.to_csv(
    "data/processed/final_without_tapping_dataset.csv",
    index=False
)

print("Preprocessing without tapping dataset completed successfully.")
print("Final Dataset Shape:", final_data.shape)
print("Saved to: data/processed/final_without_tapping_dataset.csv")