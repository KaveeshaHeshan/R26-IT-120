...
const generateMockLiveStream = () => {
  ...
  for (let i = 20; i >= 0; i--) {
    ...
    currentPH += (Math.random() - 0.5) * 0.1
    currentTurbidity += (Math.random() - 0.5) * 2
...
const LiveMonitor = () => {
  ...
  useEffect(() => {
    ...
    const handleMockFallback = () => {
      const mockData = generateMockLiveStream()
    ...
    const unsub = onValue(q, (snapshot) => {
      try {
        ...
        console.warn('Firebase error, using mock data fallback.')
        handleMockFallback()
      }
    }, (err) => {
      console.warn('Firebase error, using mock data fallback.')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/90 rounded-[2rem] p-10 text-center border border-emerald-100 relative overflow-hidden shadow-xl group">
        ...
        <div className="flex flex-col items-center relative z-10">
          <div className="flex items-center gap-3 mb-6">
            <div className="flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
              <div
                ...
                style={{ backgroundColor: getGradeColor(latest.grade), color: getGradeColor(latest.grade) }}
              />
              <span className="text-[#052c14] text-[10px] font-black uppercase tracking-[0.2em]">
                Live Stream Active
              </span>
            </div>
          </div>
          ...
          <div className="relative mb-2">
            <div
              className="text-[10rem] font-black leading-none tracking-tighter transition-all duration-500 group-hover:scale-[1.02]"
              style={{ 
            ...
            <div className="absolute -right-12 bottom-4 text-[#052c14]/10 text-xs font-black uppercase tracking-widest rotate-90">
              Units / VFA
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-6 mt-4">
            ...
            <div className="flex gap-4">
              ...
              <div className="px-5 py-2 rounded-xl bg-emerald-50 border border-emerald-50">
                <p className="text-[9px] font-black text-[#052c14]/20 uppercase tracking-widest mb-1 text-center">Stability</p>
                <p className="text-xs font-black text-[#052c14] uppercase tracking-wide text-center">
                  {(0.98 + Math.random() * 0.015).toFixed(3)}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white/90 rounded-2xl p-6 border border-emerald-100 shadow-xl relative overflow-hidden group hover:bg-emerald-50 transition-all">
          ...
          <div className="mt-6 space-y-2 relative z-10">
            ...
            <div className="bg-emerald-50 rounded-full h-2 border border-emerald-100 overflow-hidden">
              <div
                className="bg-gradient-to-r from-blue-600 to-blue-300 h-full rounded-full shadow-[0_0_15px_rgba(59,130,246,0.4)] transition-all duration-1000"
                style={{ width: `${(latest.pH / 14) * 100}%` }}
              />
            </div>
            <p className="text-[10px] font-bold text-[#052c14]/40 text-center tracking-wide pt-1 italic">
        ...
        <div className="bg-white/90 rounded-2xl p-6 border border-emerald-100 shadow-xl relative overflow-hidden group hover:bg-emerald-50 transition-all">
          ...
          <div className="mt-6 space-y-2 relative z-10">
            ...
            <div className="bg-emerald-50 rounded-full h-2 border border-emerald-100 overflow-hidden">
              <div
                className="bg-gradient-to-r from-emerald-600 to-emerald-400 h-full rounded-full shadow-[0_0_15px_rgba(16,185,129,0.4)] transition-all duration-1000"
        ...
        <div className="bg-white/90 rounded-2xl p-6 border border-emerald-100 shadow-xl relative overflow-hidden group hover:bg-emerald-50 transition-all">
          ...
          <div className="mt-6 space-y-2 relative z-10">
            ...
            <div className="bg-emerald-50 rounded-full h-2 border border-emerald-100 overflow-hidden">
              <div
                className="bg-gradient-to-r from-orange-600 to-red-400 h-full rounded-full shadow-[0_0_15px_rgba(249,115,22,0.4)] transition-all duration-1000"
                style={{ width: `${((latest.temperature - 15) / 25) * 100}%` }}
              />
            </div>
            <p className="text-[10px] font-bold text-[#052c14]/40 text-center tracking-wide pt-1 italic">
              Sensor variation: ±0.1°C
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100 shadow-inner
        ...
        {[
          { label: 'Network Node',   value: latest.farmer_id  || 'FARM-001' },
          { label: 'D-Link ID',   value: latest.device_id  || 'ESP-32-L' },
          { label: 'Batch UID',   value: latest.sample_id  || 'BATCH-X' },
          ...
          <div key={i} className="flex flex-col items-center justify-center border-r last:border-0 border-emerald-100">
            <p className="text-[#052c14]/20 text-[9px] font-black uppercase tracking-[0.2em] mb-2">
              {item.label}
            </p>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-sm font-black text-[#052c14] tracking-[0.2em] uppercase">
                VFA Dynamic Trend
              </h3>
            </div>
            <p className="text-[#052c14]/30 text-[10px] font-bold tracking-widest uppercase ml-3">
              Real-time variance analysis (20 samples)
            </p>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-[#4CBB17] shadow-[0_0_8px_#4CBB17]" />
              <span className="text-[9px] font-black text-[#052c14]/40 uppercase tracking-widest">Safe Level</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgb(239,68,68)]" />
                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-white/95 backdrop-blur-xl p-4 border border-emerald-100 rounded-xl shadow-xl">
                        ...
                        <div className="mt-2 pt-2 border-t border-emerald-100 flex gap-4">
                          <span className="text-[9px] font-bold text-[#052c14]/40">pH: {payload[0].payload.pH?.toFixed(2)}</span>
                          <span className="text-[9px] font-bold text-[#052c14]/40">Temp: {payload[0].payload.temperature?.toFixed(1)}°C</span>
                        </div>
                      </div>
                    );
                  }
      ...
      <div className="bg-white/90 rounded-2xl shadow-xl border border-emerald-100 overflow-hidden">
        <div className="px-8 py-5 border-b border-emerald-50 bg-emerald-50/50 flex items-center justify-between">
            <h3 className="text-[11px] font-black text-[#052c14]/80 tracking-[0.2em] uppercase">
              System Telemetry Logs
            </h3>
            <span className="text-[9px] font-black text-[#052c14]/20 uppercase tracking-widest italic">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-emerald-50">
                {['Registry Time','Node','VFA Metric','Stability','Clarity','Thermal','Rank'].map(h => (
                  <th key={h}
            ...
            <tbody className="divide-y divide-emerald-50">
              {[...history].reverse().slice(0, 10).map((item, i) => (
                <tr key={item.id}
                  ...
                  <td className="px-6 py-4 text-[#052c14]/80 font-black text-[10px] tracking-wider uppercase">
                    {item.farmer_id}
                  </td>
                  ...
                  <td className="px-6 py-4 text-[#052c14]/60 text-[10px] font-bold">
                    {item.pH.toFixed(2)}
                  </td>
                  <td className="px-6 py-4 text-[#052c14]/60 text-[10px] font-bold">
                    {item.turbidity.toFixed(1)} <span className="text-[8px] opacity-40">NTU</span>
                  </td>
                  <td className="px-6 py-4 text-[#052c14]/60 text-[10px] font-bold">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-[8px] font-black px-2 py-1 rounded-md border uppercase tracking-widest shadow-sm"
                      style={{ 
                        ...
                        borderColor: `${getGradeColor(item.grade)}40`,
