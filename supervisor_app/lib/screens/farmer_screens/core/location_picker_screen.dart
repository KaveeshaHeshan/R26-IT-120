...
// ================================================================
//
// Lets a farmer choose (or simply view) a GPS point on a map.
// Tap anywhere on the map or use the "my location" button to drop
// the pin, then confirm to return the LatLng to the caller (e.g. the
// tapping record form, which stores it as latitude/longitude fields).
//
// Uses flutter_map with OpenStreetMap tiles, matching the supervisor
// route screen: no Google Maps API key and no billing account needed.
// ================================================================

/// Default map center when no location has been picked yet
/// (roughly the rubber-growing wet zone of Sri Lanka).
///
/// Not `const`: latlong2's LatLng constructor validates its arguments, so it
/// cannot be evaluated at compile time.
final LatLng kDefaultFarmLocation = LatLng(6.8211, 80.1200);
              ...
              children: <Widget>[
                FlutterMap(
                  ...
                  options: MapOptions(
                    ...
                    initialZoom: 15,
                    // Tapping the map moves the pin. Dragging the marker
                    // itself is not supported by flutter_map, so tap-to-place
                    // is the single way to reposition it.
                    ...
                    MarkerLayer(
                      markers: <Marker>[
                        Marker(
                          ...
                          width: 44,
                          height: 44,
                          // Anchored so the pin's tip sits on the point.
