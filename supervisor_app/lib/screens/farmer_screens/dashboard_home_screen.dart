...
class DashboardHomeScreen extends StatelessWidget {
  ...
  Widget _alertTile(FarmerPalette p, dynamic alert) {
    if (alert is Map<String, dynamic> &&
        (alert['source'] == 'lstm_quality_forecast' || alert['source'] == 'lstm_prediction_unavailable')) {
  ...
  Stream<List<Map<String, dynamic>>> _watchLstmForecastAlerts() {
    return FirebaseFirestore.instance.collection('quality_forecasts').doc(userId).snapshots().map(
      (DocumentSnapshot<Map<String, dynamic>> snapshot) {
        ...
        final bool complete = required.every((String field) => forecast.containsKey(field) && forecast[field] != null);
        if (!complete) {
          return <Map<String, dynamic>>[
            <String, dynamic>{
              'source': 'lstm_prediction_unavailable',
              'title': 'Prediction unavailable',
              'message': 'The latest forecast is incomplete and cannot be shown safely.',
            },
          ];
        }

        return <Map<String, dynamic>>[
          <String, dynamic>{...forecast, 'source': 'lstm_quality_forecast'},
        ];
      },
    );
  }
  ...
  Widget _forecastAlertTile(FarmerPalette p, Map<String, dynamic> forecast) {
    ...
    final bool unavailable = source == 'lstm_prediction_unavailable';
      ...
      child: Column(
        ...
        children: <Widget>[
            ...
            children: <Widget>[
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text(_stringOrDefault(forecast['title'], fallback: 'Prediction unavailable'), style: TextStyle(color: p.textPrimary, fontSize: 16, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    Text(_stringOrDefault(forecast['message'], fallback: 'A complete quality forecast is not available yet.'), style: TextStyle(color: p.textSecondary, height: 1.35)),
                  ],
                ),
              ),
            ],
          ...
          if (!unavailable && _stringOrDefault(forecast['reason'], fallback: '').isNotEmpty) ...<Widget>[
            ...
            const SizedBox(height: 5),
            Text(_stringOrDefault(forecast['reason']), style: TextStyle(color: p.textSecondary, height: 1.38)),
          ],
          if (!unavailable && recommendations.isNotEmpty) ...<Widget>[
            const SizedBox(height: 15),
            Text('What should I do?', style: TextStyle(color: p.textPrimary, fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            ...recommendations.map((dynamic item) => Padding(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      Expanded(child: Text(_stringOrDefault(item), style: TextStyle(color: p.textSecondary, height: 1.3))),
                    ],
                  ),
                )),
          ],
